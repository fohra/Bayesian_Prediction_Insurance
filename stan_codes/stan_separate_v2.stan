...
parameters {
  ...
  vector<lower=0>[N] sigma_age ; //hierarchial prior sigma for age
  
  vector[N] theta_sex; //hierarchial prior mu for sex
  vector<lower=0>[N] sigma_sex ; //hierarchial prior sigma for sex
  
  vector[N] theta_bmi; //hierarchial prior mu for bmi
  vector<lower=0>[N] sigma_bmi; //hierarchial prior sigma for bmi
  vector<lower=0>[N] prior_alpha_bmi;
  
  vector[N] theta_smoker; //hierarchial prior mu for smoker
  vector<lower=0>[N] sigma_smoker; //hierarchial prior sigma for smoker
  
  vector[N] theta_insurance; //hierarchial prior mu for insurance
  vector<lower=0>[N] sigma_insurance; //hierarchial prior sigma for insurance
  vector<lower=0>[N] prior_alpha_insurance;
